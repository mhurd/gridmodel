...
class WrappingGridSpec extends FlatSpec {
  ...
  def nonEmptyFixture =
    new {
      ...
      val grid = WrappingGrid[Int](3, 3, cells)
    }

  "WrappingGrid" must "be able to get the surrounding cells of any cell" in {
    val f = nonEmptyFixture
    expect("List((0,0) = 1, (0,1) = 2, (0,2) = Empty, (1,0) = 3, (1,2) = 5, (2,0) = Empty, (2,1) = Empty, (2,2) = 6)") {
      f.grid.get(1, 1).surroundingCells.toString()
    }
  }

  it must "be able to get the surrounding cells of any cell - handling out of bounds cells by wrapping" in {
    val f = nonEmptyFixture
    expect("List((0,1) = 2, (1,0) = 3, (1,1) = 4, (1,2) = 5, (2,1) = Empty, (2,2) = 6)") {
